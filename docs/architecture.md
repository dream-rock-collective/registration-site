...
- `registered.ts` reads the registration id from the URL, starts checkout, and saves the pending plan.
- `success.ts` completes the pending plan locally.
- `allocate-payment.ts` computes the plan budget, renders and edits the allocation, and submits it.
- `member.ts` is the shared browser-state helper. Keep local-storage reads/writes there rather than duplicating the schema in page modules.
